...
router.post(
  ...
  [
    ...
    check('confirmNewPassword', 'You must confirm your new Password').exists(),
  ],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    try {
      ...
      const newPasswordsMatch = Boolean(newPassword === confirmNewPassword);
